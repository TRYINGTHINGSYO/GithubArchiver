...
describe("RelaySession orchestrator", () => {
  ...
  it("waits for plan approval before editing, then runs task graph", async () => {
    const dir = await mkdtemp(path.join(tmpdir(), "relay-"));
    const gpt = mockGpt([
      {
        status: "plan",
      ...
      {
        status: "complete",
        summary: "Stopped after plan + graph node",
        next_improvements: [],
      },
    ]);
    const cursor = mockCursor([{ ok: true, stdout: "API done" }]);
    const session = new RelaySession(baseDeps(gpt, cursor));
    ...
    expect(session.snapshot().taskGraph?.nodes?.[0]?.id).toBe("1");
    ...
    expect(cursor.run).toHaveBeenCalledTimes(1);
    expect(JSON.stringify(cursor.run.mock.calls)).toContain("Task graph node 1");
    expect(session.snapshot().taskGraph?.progress?.complete).toBe(true);
  });
