...
function rssMb(): number | null {
	try {
		const status = spawnSync('ps', ['-o', 'rss=', '-p', String(process.pid)], {
			encoding: 'utf8'
		});
...
async function indexAllViaWorker(batchSize: number) {
	...
	for (let i = 0; i < rows.length; i += batchSize) {
		...
		// Worker embeds+upserts in one call; attribute most time to embed for MiniLM CPU.
		embedMs += t1 - t0;
		upsertMs += Math.max(1, (t1 - t0) * 0.05);
	...
	return { indexed, failed, embedMs, upsertMs, syncMs, eligible: rows.length };
}
...
async function main() {
	...
	const worker = startWorker({
		...
		indexPath,
		bits: 2,
		logPath: path.join(OUT_DIR, 'worker-2bit.log')
	});
	...
	console.log('Worker ready', health);

	const qLatencies: number[] = [];
	for (let i = 0; i < 20; i++) {
		const t0 = performance.now();
		await semanticWorkerSearch({ query: 'local voice assistant', k: 10 });
		qLatencies.push(performance.now() - t0);
	}
	// first may include cold embed; measure warm separately after indexing too
	...
	const indexReport = {
		...
		indexing: {
			...
			embed_proxy_s: indexStats.embedMs / 1000,
			sync_s: indexStats.syncMs / 1000,
			index_bytes: stats?.indexBytes ?? null,
			model_load_s: modelLoadMs / 1000,
			peak_rss_mb_node: rssAfter,
	...
	for (const q of EVAL_QUERIES) {
		...
		queryWarm.push(performance.now() - t0);
	...
	const filterCases: Array<{
		...
		{ name: 'language=Python', query: 'offline speech recognition wake word', opts: { language: 'Python' }, expectMin: 1 },
		{ name: 'minStars=500', query: 'download manager torrent', opts: { minStars: 500 }, expectMin: 1 },
		{ name: 'maxStars=50', query: 'utility formatting JSON logs', opts: { maxStars: 50 }, expectMin: 1 },
		{ name: 'category=networking', query: 'network monitoring dashboard', opts: { category: 'networking' }, expectMin: 1 },
		{ name: 'cluster=ops', query: 'infrastructure monitoring', opts: { cluster: 'ops' }, expectMin: 1 },
		{
		...
		{ name: 'hasReadme', query: 'GitHub backup archive', opts: { hasReadme: true }, expectMin: 1 },
		{ name: 'hasRelease', query: 'GitHub backup archive', opts: { hasRelease: true }, expectMin: 1 },
		{ name: 'archivedOnly', query: 'GitHub backup archive', opts: { archivedOnly: true }, expectMin: 1 },
		{
	...
	for (const fc of filterCases) {
		if (fc.softMax != null) {
			process.env.SEMANTIC_ALLOWLIST_SOFT_MAX = String(fc.softMax);
		} else {
			process.env.SEMANTIC_ALLOWLIST_SOFT_MAX = '80';
		}
		...
		for (const repo of result.repos) {
			if (fc.opts.language && repo.language !== fc.opts.language) {
				leaks.push(`language:${repo.full_name}`);
			}
			if (fc.opts.minStars != null && (repo.stars ?? 0) < Number(fc.opts.minStars)) {
				leaks.push(`minStars:${repo.full_name}`);
			}
			if (fc.opts.maxStars != null && (repo.stars ?? 0) > Number(fc.opts.maxStars)) {
				leaks.push(`maxStars:${repo.full_name}`);
			}
			if (fc.opts.category && repo.category !== fc.opts.category) {
	...
	const worker2 = startWorker({
		...
		indexPath,
		bits: 2,
		logPath: path.join(OUT_DIR, 'worker-2bit-restart.log')
	});
	...
	const worker3 = startWorker({
		...
		indexPath,
		bits: 2,
		logPath: path.join(OUT_DIR, 'worker-2bit-after-remove.log')
	});
	await waitForWorker(workerUrl);
	...
	const docPayload = {
		...
		bits: [2, 3, 4],
		out_dir: path.join(OUT_DIR, 'bits')
	};
	mkdirSync(docPayload.out_dir, { recursive: true });
	const dumpPath = path.join(OUT_DIR, 'bit-eval-input.json');
	...
	const quality = {
		...
		regressions,
		query_latency_ms: {
			p50: percentile(queryWarm, 50),
			p95: percentile(queryWarm, 95),
			mean: mean(queryWarm)
		}
	};
...
function buildRecommendation(input: {
	quality: {
		...
		regressions: unknown[];
		query_latency_ms: { p50: number; p95: number };
	};
	indexReport: { indexing: { indexed: number; failed: number; index_bytes: number | null; wall_clock_s: number; repos_per_sec: number } };
	weightResults: unknown[];
	bitReport: unknown;
	filterResults: unknown[];
	restartReport: { removed_still_absent: boolean; indexed_before: number | null; indexed_after_restart: number | null };
}) {
	...
	const bestWeight = (input.weightResults as Array<{
	...
	return {
		...
		default_weights_note:
			'Do not change defaults from this run alone unless evidence is overwhelming; current 0.55/0.35/0.10 remains recommended unless bestWeight clearly dominates on real production data.',
		observed_best_weights: bestWeight ?? null,
		vector_bits_default: 2,
		vector_bits_note:
			'2-bit remains sensible for production (smallest index, sub-ms queries in microbench). Re-check recall vs exact on full production archive before raising bits.',
		railway: {
			worker_private: true,
			expose_publicly: false,
			suggested_resources:
				'Start with 1 vCPU / 2GB RAM private service for ≤~100k repos; plan 4GB+ and dedicated disk for multi-hundred-thousand archives. Keep indexBatch/remove/rebuild on localhost/private network only.',
...
function renderMarkdown(full: {
	...
	indexReport: {
		...
		indexing: Record<string, number | null | Record<string, number>>;
	...
	quality: {
		...
		ties: number;
		regressions: unknown[];
		query_latency_ms: { p50: number; p95: number; mean: number };
	};
	perQuery: Array<{
		id: string;
		query: string;
	...
	lines.push('');
	lines.push(
		`Query latency (hybrid/semantic path wall): p50=${full.quality.query_latency_ms.p50.toFixed(1)}ms p95=${full.quality.query_latency_ms.p95.toFixed(1)}ms`
	);
	lines.push('');
