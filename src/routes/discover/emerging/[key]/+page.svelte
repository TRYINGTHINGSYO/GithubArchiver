<script lang="ts">
	...
	const staleTopic = $derived(data.staleTopic);
	const topic = $derived(detail?.topic);
	const evidence = $derived(detail?.evidence);
	...
	const primaryRepositories = $derived(
		detail
			? primaryRepositoryIds.size > 0
				? detail.repositories.filter((repo) => primaryRepositoryIds.has(repo.id))
				: detail.repositories
			: []
	);
	...
	const pageTitle = $derived(
		isStale && staleTopic
			? `${staleTopic.label} - Awaiting recomputation - GithubArchive+`
			: topic
				? `${topic.label} - Emerging topic - GithubArchive+`
				: 'Emerging topic - GithubArchive+'
	);
...
{#if isStale && staleTopic}
	<section class="topic-hero stale-topic">
		<a class="back" href="/discover/emerging">Back to emerging topics</a>
		<div class="hero-head">
			<div>
				<p class="eyebrow">awaiting recomputation</p>
				<h1>{staleTopic.label}</h1>
			</div>
			<div class="score-box">
				<strong>v{staleTopic.detectionVersion}</strong>
				<span>Stored model</span>
			</div>
		</div>
		<p>
			This topic was generated by an older detection model and is awaiting recomputation. It is
			not being shown as current intelligence until the detector produces a fresh v{staleTopic.currentVersion}
			result.
		</p>
		<div class="metrics">
			<span>Current model v{staleTopic.currentVersion}</span>
			<span>Status {staleTopic.status}</span>
			<span>Generated {staleTopic.generatedAt.slice(0, 10)}</span>
			<span>Period {staleTopic.periodStart.slice(0, 10)} to {staleTopic.periodEnd.slice(0, 10)}</span>
		</div>
		<p class="hint">
			Run the emerging-topic detector to regenerate this topic with independent evidence scoring
			and duplicate-family analysis.
		</p>
	</section>
{:else if topic && detail && evidence}
