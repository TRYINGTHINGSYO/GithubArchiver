...
Return ONLY valid JSON:
{
  ...
  "plan": {
    ...
    "steps": [{
      ...
      "title":"...",
      "detail":"...",
      "role":"backend|frontend|tests|docs",
      "dependsOn":["t0"],
      "agentHint":"cursor|claude-code|openai-codex"
    }],
    "estimatedMinutes": 12,
...
Rules:
- On the first turn (or when asked to plan), use status=plan with a concrete plan / task graph. Do not edit yet.
- Prefer explicit dependsOn edges so independent work can run in parallel after prerequisites pass.
- After the plan is approved, use continue or parallel for remaining free-form work; the orchestrator also executes the task graph node-by-node.
    ...
    .map((s, i) => {
      ...
      return {
        ...
        dependsOn: Array.isArray(o.dependsOn)
          ...
          : undefined,
        agentHint: typeof o.agentHint === "string" ? o.agentHint : undefined,
      };
    })
